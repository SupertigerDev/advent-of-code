...
for (let i = 0; i < lines.length; i++) {
    ...
    const moveCount = parseInt(splitMessage[1]);
    ...
    let itemsToMove = cloneStack.filter((_, i) => i < moveCount).reverse();
...
function parseStacks() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let y = 0; y < rows.length; y++) {
            ...
            if (y % 4 === 3) continue;
            index++;
            !rowStack[i] ? rowStack[i] = [column] : rowStack[i].push(column);
        }
    }
    const columnStack = [];

    for (let r = 0; r < rowStack.length; r++) {
        const rows = rowStack[r];
        for (let c = 0; c < rows.length; c++) {
            const column = rows[c];
            if (!column.trim()) continue;
            !columnStack[c] ? columnStack[c] = [column] : columnStack[c].push(column);
        }
    }
    return columnStack;
}
